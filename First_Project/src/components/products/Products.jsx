import React, { useState, useEffect } from "react";
import "./Product.css";
import { getProducts } from "./Product.js";
import {Link} from 'react-router-dom';

let intervalId;

function Products() {
  // storing total products
  const [products, setProducts] = useState([]);
  // filtered products
  const [filter, setFilter] = useState([]);
  // value for interval
  var [current, setCurrent] = useState(0);
  var [active, setActive] = useState("inactive");

  const slides = [
    {
      image:
        "https://t3.ftcdn.net/jpg/13/34/41/12/360_F_1334411297_kzhxaQGiMizH7x14KuRrfi3UhK4IUzBm.jpg",
      title: "Electronics",
      description:
        "Products. Consumer electronics devices include those used for: Entertainment (Flatscreen TVs, television sets, MP3 players, video recorders, DVD players, radio receivers, etc.) Communications (telephones, mobile phones, email-capable personal computers, desktop computers, laptops, printers, paper shredders, etc.)",
    },
    {
      image:
        "https://rapaport.com/wp-content/uploads/2024/07/1280x720-1-20240701122015.jpg",
      title: "Jewelry",
      description:
        "ornamental pieces (such as rings, necklaces, earrings, and bracelets) that are made of materials which may or may not be precious (such as gold, silver, glass, and plastic), are often set with genuine or imitation gems, and are worn for personal adornment. a collection of valuable jewelry. a jewelry store",
    },
    {
      image:
        "https://www.manyavar.com/on/demandware.static/-/Library-Sites-ManyavarSharedLibrary/default/dw0ecd2c4f/wedding%20blogs/Exploring%20All%20Kinds%20of%20Indian%20Menswear_D%20copy.jpg",
      title: "Men's Clothing",
      description:
        "Men's wear, or menswear, is a category of clothing and accessories designed for men. It encompasses a wide variety of items, from formal wear like suits and dress shirts to casual clothing such as jeans and t-shirts, as well as sportswear for active pursuits and ethnic attire for cultural events",
    },
    {
      image:
        "https://rangita.com/cdn/shop/articles/ethnic_wear_blog_banner_e8ea06d7-94ef-4dba-9cc2-f5f6420840a5.png?v=1748513115",
      title: "Women's Clothing",
      description:
        "Women's clothing involves details about its style, such as minimalist, bohemian, or streetwear, and its fit (e.g., tight-fitting, baggy). Key features to include are the color, pattern, fabric (like lace or silk), and overall silhouette.",
    },
  ];

  if (!intervalId) {
    intervalId = setInterval(() => {
      setCurrent((data) => {
        if (data === slides.length - 1) {
          return 0;
        } else {
          return data + 1;
        }
      });
    }, 3000);
  }

  useEffect(() => {
    getProducts(setProducts);
    getProducts(setFilter);
  }, []);

  function getAllProducts() {
    console.log("Msg");
    setFilter(products);
  }

  function getElectronics(categoryName) {
    var electronic = products.filter((p) => p.category === categoryName);
    console.log(electronic);
    setFilter(electronic);
  }

  function getJewelry(categoryName) {
    var jewelry = products.filter((p) => p.category === categoryName);
    setFilter(jewelry);
  }

  function getMensClothing(categoryName) {
    var mens = products.filter((p) => p.category === categoryName);
    console.log(mens);
    setFilter(mens);
  }
  // "jewelery","men's clothing","women's clothing"
  function getWomensClothing(categoryName) {
    var womens = products.filter((p) => p.category === categoryName);
    setFilter(womens);
  }

  return (
    <div>
      <section
        style={{
          backgroundImage: `linear-gradient(45deg, rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url(${slides[current].image})`,
        }}
        className="product-banner"
      >
        <h2>{slides[current].title}</h2>
        <p>{slides[current].description}</p>
      </section>
      <section className="product-category">
        <div
          onClick={() => {
            setActive("all");
            getAllProducts();
          }}
          style={{ cursor: "pointer" }}
        >
          <img
            className={active === "all" ? "active" : "inactive"}
            src="https://eatvita.co.uk/wp-content/uploads/2022/04/all-products.svg"
            alt=""
            height={80}
            width={80}
          />
          <h6>All</h6>
        </div>
        <div
          // "jewelery","men's clothing","women's clothing"
          onClick={() => {
            setActive("electronics");
            getElectronics("electronics");
          }}
          style={{ cursor: "pointer" }}
        >
          <img
            className={active === "electronics" ? "active" : "inactive"}
            src="https://flare.pk/wp-content/uploads/2023/01/1-1.jpg"
            alt=""
            height={80}
            width={80}
          />
          <h6>Electronic</h6>
        </div>
        <div
          // "jewelery","men's clothing","women's clothing"
          onClick={() => {
            setActive("jewelery");
            getJewelry("jewelery");
          }}
          style={{ cursor: "pointer" }}
        >
          <img
            className={active === "jewelery" ? "active" : "inactive"}
            src="https://static.wixstatic.com/media/bb2928_d6e78300457046168a3d27164230c187~mv2.jpg/v1/fill/w_824,h_430,al_c,q_80,usm_0.66_1.00_0.01,enc_avif,quality_auto/May%2520Emerald%2520plant%2520(1)_edited.jpg"
            alt=""
            height={80}
            width={80}
          />
          <h6>Jewelry</h6>
        </div>
        <div
          onClick={() => {
            setActive("men's clothing");
            getMensClothing("men's clothing");
          }}
          style={{ cursor: "pointer" }}
        >
          <img
            className={active === "men's clothing" ? "active" : "inactive"}
            src="data:image/jpeg;base64,"
            alt=""
            height={80}
            width={80}
          />
          <h6>Men's</h6>
        </div>
        <div
          onClick={() => {
            setActive("women's clothing");
            getWomensClothing("women's clothing");
          }}
          style={{ cursor: "pointer" }}
        >
          <img
            className={active === "women's clothing" ? "active" : "inactive"}
            src="https://img.faballey.com/images/Product/XKS03763F/d3.jpg"
            alt=""
            height={80}
            width={80}
          />
          <h6>Women's</h6>
        </div>
      </section>
      <section className="product-data">
        {filter.length == 0 ? (
          <p>No Products Found</p>
        ) : (
          filter.map(function (product) {
            return (
              <div className="card">
                <img src={product.image} alt="" width="100%" height={200} />
                <h5>{product.title}</h5>
                <p>${product.price}</p>
                <h6>{product.category}</h6>
                <div className="card-button">
                  <Link to={`/product-details/${product.id}`}>
                    <button >Product Details</button>
                  </Link>
                  <button>Add-To-Card</button>
                </div>
              </div>
            );
          })
        )}
      </section>
    </div>
  );
}

export default Products;
